...
window.document.querySelectorAll<HTMLBodyElement>(`[data-component="body"]`).forEach((bodyElement) => {
    // Go over each menu toggle button.
    bodyElement.querySelectorAll<HTMLButtonElement>(`button[data-body="menu-toggle"]`).forEach((menuToggleButton) => {
        // Add an event listener for clicking the menu toggle button.
        menuToggleButton.addEventListener("click", () => {
            // Get the current menu toggle state.
            const currentIsToggled: boolean = bodyElement.getAttribute("data-body-is-menu-toggled") === "true";
            // Update the menu toggle state.
            bodyElement.setAttribute("data-body-is-menu-toggled", `${!currentIsToggled}`);
        });
    });
    // Go over each menu backdrop element.
    bodyElement.querySelectorAll<HTMLDivElement>(`div[data-body="menu-backdrop"]`).forEach((menuBackdropElement) => {
        // Add an event listener for clicking the menu backdrop element.
        menuBackdropElement.addEventListener("click", () => {
            // Update the menu toggle state.
            bodyElement.setAttribute("data-body-is-menu-toggled", "false");
        });
    });
});
